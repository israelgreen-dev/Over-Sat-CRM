...
export default function TargetsTab({
  ...
  function deleteRow(name: string, id: string) {
    handleManagerRows(
      name,
      (productTargetRowsByManager[name] ?? []).filter((r) => r.id !== id),
    )
  }

  const annualOf = (name: string) =>
    Math.round(computeWeighted(productTargetRowsByManager[name] ?? []))
  ...
  function deriveQuarters(
    annual: number,
    vals: Record<QKey, number>,
    manual: QKey[],
  ): QuarterlyData {
    ...
    if (unpinned.length > 0) {
      ...
      unpinned.forEach((k, i) => {
        out[k] = base + (i === unpinned.length - 1 ? leftover : 0)
      })
    }
  ...
  function handleQuarter(name: string, q: QKey, v: number | null) {
    ...
    onQuarterlyTargetsChange({
      ...quarterlyTargets,
      [name]: deriveQuarters(annualOf(name), vals, manual),
    })
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-400">
            Overall Target:{' '}
            <span className="font-semibold text-gray-700">{fmt(overallTarget)}</span>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-50 bg-white">
                    <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 min-w-[150px]">
                      Product
                    </th>
                    <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 min-w-[180px]">
                      Description
                    </th>
                    <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500 min-w-[120px]">
                      Unit Price ($)
                    </th>
                    <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500 min-w-[72px]">
                      Qty
                    </th>
                    <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-blue-500 min-w-[110px]">
                      Total Price ($)
                    </th>
                    <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-purple-500 min-w-[110px]">
                      Probability
                    </th>
                    <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-emerald-600 min-w-[120px]">
                      Weighted Price ($)
                    </th>
                    <th className="w-9" />
                  </tr>
                ...
                <tbody className="divide-y divide-gray-50">
                        ...
                        <td className="px-4 py-2">
                          {readOnly ? (
                            ...
                            <select
                              value={row.product}
                              onChange={(e) =>
                                updateRow(name, row.id, 'product', e.target.value)
                              }
                        ...
                        <td className="px-3 py-2">
                          {readOnly ? (
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateRow(name, row.id, 'description', e.target.value)
                              }
                        ...
                        <td className="px-3 py-2">
                          {readOnly ? (
                            ...
                            <NumericInput
                              ...
                              value={row.quantity}
                              onChange={(v) =>
                                updateRow(name, row.id, 'quantity', Math.max(1, v ?? 1))
                              }
                            />
                          )}
                        </td>
                  ...
                  {rows.length > 0 && (
                    <tr className="border-t-2 border-gray-100 bg-slate-50 font-bold">
                      <td
                        className="px-4 py-2.5 text-xs uppercase tracking-wider text-gray-400"
                        colSpan={4}
                      >
                        Total
                      </td>
                ...
                <thead>
                  <tr className="border-b border-gray-50 bg-amber-50/40">
                    ...
                    {QUARTERS.map((q) => (
                      <th
                        key={q}
                        className="px-3 py-2 text-center text-xs font-semibold uppercase tracking-wider text-amber-500 min-w-[110px]"
                      >
                        {Q_LABELS[q]}
                      </th>
                    ))}
                ...
                <tbody>
                    ...
                    {QUARTERS.map((q) => {
                      ...
                      return (
                        <td key={q} className="px-3 py-2 w-28 align-top">
                          {readOnly ? (
                            <span className="block text-center text-sm font-semibold text-amber-700">
                              {fmtShort(qt[q])}
                            </span>
                          ) : (
                            <div className="flex flex-col gap-0.5">
                              ...
                              <span
                                ...
                                title={
                                  isManual
                                    ? 'Manually set — pinned'
                                    : 'Auto — shares the remainder evenly'
                                }
                              >
                  ...
                  {annualTarget > 0 &&
                    (() => {
                      ...
                      const pct = Math.min(
                        100,
                        annualTarget > 0 ? (qSum / annualTarget) * 100 : 0,
                      )
                        ...
                        <tr className="bg-white">
                          ...
                          <td colSpan={4} className="px-3 pb-3 pt-1">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <span
                                className={`text-xs font-semibold ${isMatch ? 'text-emerald-700' : isOver ? 'text-red-700' : 'text-amber-700'}`}
                              >
                                {isMatch
                              ...
                              <span
                                className={`ml-auto text-xs font-bold ${isMatch ? 'text-emerald-600' : isOver ? 'text-red-600' : 'text-amber-600'}`}
                              >
                                {Math.round(pct)}%
                              </span>
                            </div>
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 min-w-[120px]">
                  Company Total
                </th>
                ...
                {QUARTERS.map((q) => (
                  <th
                    key={q}
                    className="px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider text-amber-500"
                  >
                    {Q_LABELS[q]} Target
                  </th>
                ))}
            ...
            <tbody>
              <tr className="font-bold">
                ...
                {QUARTERS.map((q) => (
                  <td key={q} className="px-3 py-3 text-center text-amber-600">
                    {fmtShort(overallQ[q])}
                  </td>
                ))}
              </tr>
      ...
      {managers.length > 0 && (
        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400">
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-blue-300" /> Total Price — unit price ×
            qty
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" /> Weighted Price — total
            × probability %
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-amber-300" /> Quarters — auto ÷4 by
            default; edit one to pin it and the rest auto-share the remainder
          </span>
        </div>
      )}
    </div>
