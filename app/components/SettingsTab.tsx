...
export default function SettingsTab({
  ...
  onOverallTargetChange: (v: number) => void
  ...
  return (
    <div className="space-y-8">
      ...
      <TargetsEditor
        ...
        onOverallTargetChange={onOverallTargetChange}
...
function TargetsEditor({
  ...
  function handleOverall(raw: string) {
    const n = parseFloat(raw)
    if (!isNaN(n)) onOverallTargetChange(n)
  }

  function handleManager(name: string, raw: string) {
    const n = parseFloat(raw)
    onManagerTargetsChange({ ...managerTargets, [name]: isNaN(n) ? 0 : n })
  }
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      <div className="mb-1 flex items-center gap-2">
        ...
        <h3 className="text-sm font-bold text-gray-900">Targets (USD)</h3>
      </div>
      <p className="mb-5 text-xs text-gray-400">
        Set the overall sales target and individual quotas per manager.
      </p>
      ...
      <div className="mb-5">
        ...
        <input
          ...
          className={inputCls}
          value={overallTarget}
          onChange={(e) => handleOverall(e.target.value)}
        />
      </div>
        ...
        {managers.map((name) => (
          <div key={name} className="flex items-center gap-3">
            ...
            <input
              ...
              onChange={(e) => handleManager(name, e.target.value)}
            />
          </div>
        ))}
        {managers.length === 0 && (
