...
import { type Opportunity, DEFAULT_PROBABILITY } from '@/lib/opportunity'
...
function weightedFromRows(rows: ProductTargetRow[] | undefined): number {
  return Math.round(
    (rows ?? []).reduce(
      (s, r) => s + (r.price || 0) * (r.quantity || 0) * ((r.probability || 0) / 100),
      0,
    ),
  )
}
...
export default function Dashboard() {
  ...
  const [team, setTeam] = useState<
    { name: string; role: string; alsoManager?: boolean }[] | null
  >(null)
      ...
      team
        ? team
            .filter(
              (t) =>
                t.role === 'manager' ||
                (t.alsoManager && (t.role === 'admin' || t.role === 'head_of_sales')),
            )
            .map((t) => t.name)
        : storedManagers,
    [team, storedManagers],
  ...
  const headOfSales = useMemo(
    () =>
      team
        ? (team.find((t) => t.role === 'head_of_sales')?.name ?? storedHeadOfSales)
        : storedHeadOfSales,
    [team, storedHeadOfSales],
  )
  const [probabilityDefaults, setProbabilityDefaults] = useState<Record<string, number>>({
    ...DEFAULT_PROBABILITY,
  })
  ...
  const [quarterlyTargetsByYear, setQuarterlyTargetsByYear] = useState<
    Record<string, Record<string, QuarterlyData>>
  >({})

  // ── Per-year, per-manager product target rows ─────────────────────────────
  const [productTargetRowsByYear, setProductTargetRowsByYear] = useState<
    Record<string, Record<string, ProductTargetRow[]>>
  >({})
  ...
  const targetsForYear = useCallback(
  ...
  const setManagerTargets = useCallback(
    (v: Record<string, number>) => setTargetsByYear((prev) => ({ ...prev, [selectedYear]: v })),
    [selectedYear],
  )
  ...
  useEffect(() => {
    loadSettings().then((s) => {
      ...
      if (s.probabilityDefaults)
        setProbabilityDefaults({ ...DEFAULT_PROBABILITY, ...s.probabilityDefaults })
      ...
      if (s.notificationSettings)
        setNotificationSettings(normalizeNotificationConfig(s.notificationSettings))
  ...
  const quarterlyForYear = useCallback(
    (year: string): Record<string, QuarterlyData> => {
    ...
    const extra = Array.from(
      new Set(
        liveOpps
          ...
          .filter(
            (owner) =>
              owner && !managersLower.includes(owner.toLowerCase()) && excludeHos(owner),
          ),
      ),
    )
  ...
  const managerColors = useMemo(
    () =>
      Object.fromEntries(
        managers.map((n, i) => [
          n,
          managerColorOverrides[n] ?? COLOR_PALETTE[i % COLOR_PALETTE.length],
        ]),
      ),
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-slate-900 p-8">
        ...
        <p className="max-w-md text-center text-xs text-slate-500">
          If retrying doesn&apos;t help, signing out clears the session token and usually
          resolves token-related errors.
        </p>
      </div>
    )
  }
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="border-l border-gray-300 pl-4">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {isHoS ? `Full Access — ${headOfSales}` : `View — ${viewAs}`} &nbsp;·&nbsp;
                  Year: {selectedYear}
                </p>
              </div>
            </div>
              ...
              {!isLockedView && (
                <div className="relative" ref={viewerRef}>
                  ...
                  >
                    ...
                    <svg
                      className={`h-3.5 w-3.5 text-gray-400 transition-transform ${viewerOpen ? 'rotate-180' : ''}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
          ...
          <div className="mx-auto flex max-w-screen-xl items-center gap-4 overflow-x-auto px-6 py-3">
            <div className="inline-flex shrink-0 gap-1 rounded-2xl border border-gray-200 bg-gray-100 p-1">
              {tabs.map((tab) => (
                ...
                >
                  {safeTab !== tab && (
                    <span className={`h-1.5 w-1.5 rounded-full ${TAB_STYLE[tab].dot}`} />
                  )}
                  {tab}
                </button>
              ))}
        ...
        <main className="mx-auto max-w-screen-xl space-y-6 px-6 py-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-slate-400">
                    {isHoS
                      ? `${headOfSales} — All Teams`
                      : isPartner
                        ? 'Partner View — All Teams'
                        : 'Manager View'}
                  </p>
                  ...
                  {[
                    {
                      ...
                      value: fmtCurrency(bannerTarget),
                      accent: 'border-blue-500',
                    },
                    {
                      label: 'Total Forecast',
                      value: fmtCurrency(totalForecast),
                      accent: 'border-indigo-400',
                    },
                    {
                      label: 'Win',
                      value: fmtCurrency(closedOrders),
                      accent: 'border-emerald-400',
                    },
                    {
                      label: 'Open Pipeline',
                      value: fmtCurrency(openPipeline),
                      accent: 'border-orange-400',
                    },
                    {
                      label: 'Opportunities',
                      value: String(visibleOpps.length),
                    ...
                    <div
                      key={label}
                      className={`min-w-[110px] rounded-xl border-t-2 bg-white/10 px-4 py-3 text-center backdrop-blur-sm ${accent}`}
                    >
          ...
          {safeTab === 'Opportunities' && (
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              ...
              <PipelineTab
                ...
                defaultOwner={
                  managers.find(
                    (m) => m.toLowerCase() === (profile?.name ?? '').toLowerCase(),
                  ) ??
                  managers.find((m) =>
                    (profile?.name ?? '').toLowerCase().endsWith(m.toLowerCase()),
                  ) ??
                  profile?.name ??
                  ''
                }
          ...
          {safeTab === 'Targets' &&
            ...
            (selectedYear === ALL_YEARS ? (
              <div className="rounded-2xl border border-gray-100 bg-white px-5 py-12 text-center text-sm text-gray-400 shadow-sm">
                Targets are set per year. Select a specific year from the header to view or edit
                targets.
              </div>
            ) : (
              <TargetsTab
                managers={
                  isAdmin && !isHoS
                    ...
                    : managers.filter(
                        (m) =>
                          hosIsAlsoManager || m.toLowerCase() !== headOfSales.toLowerCase(),
                      )
                }
                managerTargets={managerTargets}
                ...
                onCopyTargetsToYear={(toYear) => {
                  ...
                  setProductTargetRowsByYear((prev) => {
                    ...
                    for (const [m, rows] of Object.entries(src))
                      copy[m] = rows.map((r) => ({
                        ...r,
                  ...
                  setQuarterlyTargetsByYear((prev) => ({
                    ...prev,
                    [toYear]: { ...(prev[selectedYear] ?? {}) },
                  }))
                }}
                products={products}
              ...
              <div>
                <div className="mb-4">
                  ...
                  <p className="mt-0.5 text-sm text-gray-400">
                    ...
                    <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-purple-100 px-2 py-0.5 text-xs font-semibold text-purple-700">
                      Admin only
                    </span>
                  </p>
                </div>
          ...
          {safeTab === 'Analytics' && (
            <AnalyticsTab
              ...
              managerTargets={
                isAdmin && !isHoS ? { [viewAs]: managerTargets[viewAs] ?? 0 } : managerTargets
              }
              quarterlyTargets={quarterlyTargets}
