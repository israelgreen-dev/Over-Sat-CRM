...
function ProjectionTab({
  ...
  const yearOptions = useMemo(
    () => Array.from(new Set(quarters.map((q) => q.split('-')[1]))),
    [quarters],
  )
  ...
  const managerOptions = useMemo(
    () =>
      Array.from(
        new Set(projectable.map((o) => (o.owner as string) ?? '').filter(Boolean)),
      ).sort(),
    [projectable],
  )
  ...
  const productOptions = useMemo(
    () =>
      Array.from(
        new Set(
          projectable.flatMap((o) => getProductLines(o).map((l) => l.product)).filter(Boolean),
        ),
      ).sort(),
    [projectable],
  )
  ...
  const filtered = useMemo(
    () =>
      projectable.filter((o) => {
        ...
        if (filterManager && (o.owner as string)?.toLowerCase() !== filterManager.toLowerCase())
          return false
        if (
          filterProduct &&
          !getProductLines(o).some(
            (l) => l.product.toLowerCase() === filterProduct.toLowerCase(),
          )
        )
          return false
  ...
  const managerRows = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      if (!map[name])
        map[name] = { perQuarter: visibleQuarters.map(() => 0), total: 0, deals: 0 }
      r.perQuarter.forEach((v, i) => {
        map[name].perQuarter[i] += v
      })
  ...
  const quarterTotals = visibleQuarters.map((_, i) =>
    rows.reduce((s, r) => s + r.perQuarter[i], 0),
  )
  ...
  function exportProjectionCSV() {
    ...
    const totalsRow = [
      ...
      '',
      '',
      ...quarterTotals.map((v) => Math.round(v)),
    ...
    a.href = url
    a.download = `sales-projection-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-gray-900">
            Sales Projection — {quarters[0]} to {quarters[quarters.length - 1]}
          </h2>
          <p className="mt-0.5 text-sm text-gray-400">
            Planned income per quarter, from the &quot;Planned Income by Quarter&quot; breakdown
            on each opportunity. Lost deals are excluded. Print (header button) or export CSV
            for a report.
          </p>
        </div>
        <button
      ...
      <div className="no-print flex flex-wrap items-center gap-x-6 gap-y-3 rounded-2xl border border-gray-100 bg-white px-5 py-3.5 shadow-sm">
        ...
        {anyFilterActive && (
          <span className="flex items-center gap-3 text-xs text-gray-400">
            ...
            <button onClick={clearAllFilters} className="transition-colors hover:text-gray-700">
              Clear all
            </button>
          </span>
        )}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm border-t-4 border-indigo-500">
          ...
          <p className="tabular-nums text-3xl font-bold leading-none text-indigo-600">
            {fmtFull(weightedTotal)}
          </p>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 mt-0.5 text-xs text-gray-400">
          Planned = sum of quarterly allocations · Weighted = adjusted by each deal&apos;s
          probability
        </p>
        {grandTotal === 0 ? (
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 mt-0.5 text-xs text-gray-400">
          Planned income per manager per quarter (USD)
        </p>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="whitespace-nowrap px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">
                    Deals
                  </th>
                  {visibleQuarters.map((q) => (
                    <th
                      key={q}
                      className="whitespace-nowrap px-3 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500"
                    >
                      {q}
                    </th>
                  ))}
                  <th className="whitespace-nowrap px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-700">
                    Total
                  </th>
                </tr>
              </thead>
                ...
                {managerRows.map((m) => (
                  <tr key={m.name} className="hover:bg-gray-50">
                    ...
                    {m.perQuarter.map((v, i) => (
                      <td
                        key={i}
                        className={`whitespace-nowrap px-3 py-2.5 text-right tabular-nums ${v > 0 ? 'text-gray-900' : 'text-gray-300'}`}
                      >
                        {v > 0 ? fmtShort(v) : '—'}
                      </td>
                    ))}
                    <td className="whitespace-nowrap px-4 py-2.5 text-right font-bold tabular-nums text-indigo-600">
                      {fmtShort(m.total)}
                    </td>
                  </tr>
                ))}
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  ...
                  {quarterTotals.map((v, i) => (
                    <td
                      key={i}
                      className="whitespace-nowrap px-3 py-3 text-right font-bold tabular-nums text-gray-900"
                    >
                      {v > 0 ? fmtShort(v) : '—'}
                    </td>
                  ))}
                  <td className="whitespace-nowrap px-4 py-3 text-right font-bold tabular-nums text-indigo-700">
                    {fmtShort(grandTotal)}
                  </td>
                </tr>
              </tfoot>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 mt-0.5 text-xs text-gray-400">
          Planned income per opportunity per quarter (USD)
        </p>
        {rows.length === 0 ? (
          <EmptyState filtered={anyFilterActive} />
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="whitespace-nowrap px-3 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">
                    Stage
                  </th>
                  {visibleQuarters.map((q) => (
                    <th
                ...
                {rows.map(({ opp, perQuarter, total }) => (
                  <tr key={opp.id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-3 py-2.5">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${STAGE_BADGE[opp.stage] ?? 'bg-gray-100 text-gray-600'}`}
                      >
                        {opp.stage ?? '—'}
                      </span>
                    </td>
        ...
        {withoutBreakdown.length > 0 && (
          <div className="mt-4 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs text-amber-500">
              Open a deal from the Opportunities tab and fill in &quot;Planned Income by
              Quarter&quot; to include it.
            </p>
          </div>
        )}
      </div>
