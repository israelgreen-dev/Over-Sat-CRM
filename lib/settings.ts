...
function readLS(): Partial<CRMSettings> {
  try {
    ...
    const m = localStorage.getItem(LS.managers)
    if (m) out.managers = JSON.parse(m)
    const p = localStorage.getItem(LS.products)
    if (p) out.products = JSON.parse(p)
    const h = localStorage.getItem(LS.headOfSales)
    if (h) out.headOfSales = h
    const pa = localStorage.getItem(LS.partners)
    if (pa) out.partners = JSON.parse(pa)
    const tby = localStorage.getItem(LS.targetsByYear)
    if (tby) out.targetsByYear = JSON.parse(tby)
    const qty = localStorage.getItem(LS.quarterlyTargetsByYear)
    if (qty) out.quarterlyTargetsByYear = JSON.parse(qty)
    const ptr = localStorage.getItem(LS.productTargetRowsByYear)
    if (ptr) out.productTargetRowsByYear = JSON.parse(ptr)
    const pd = localStorage.getItem(LS.probabilityDefaults)
    if (pd) out.probabilityDefaults = JSON.parse(pd)
    const mc = localStorage.getItem(LS.managerColors)
    if (mc) out.managerColors = JSON.parse(mc)
    const mt = localStorage.getItem(LS.managerTerritories)
    if (mt) out.managerTerritories = JSON.parse(mt)
    const ns = localStorage.getItem(LS.notificationSettings)
    if (ns) out.notificationSettings = JSON.parse(ns)
    return out
  } catch {
    return {}
  }
...
function writeLS(s: Partial<CRMSettings>): void {
  try {
    ...
    if (s.targetsByYear !== undefined)
      localStorage.setItem(LS.targetsByYear, JSON.stringify(s.targetsByYear))
    if (s.quarterlyTargetsByYear !== undefined)
      localStorage.setItem(LS.quarterlyTargetsByYear, JSON.stringify(s.quarterlyTargetsByYear))
    if (s.productTargetRowsByYear !== undefined)
      localStorage.setItem(
        LS.productTargetRowsByYear,
        JSON.stringify(s.productTargetRowsByYear),
      )
    if (s.probabilityDefaults !== undefined)
      localStorage.setItem(LS.probabilityDefaults, JSON.stringify(s.probabilityDefaults))
    if (s.managerColors !== undefined)
      localStorage.setItem(LS.managerColors, JSON.stringify(s.managerColors))
    if (s.managerTerritories !== undefined)
      localStorage.setItem(LS.managerTerritories, JSON.stringify(s.managerTerritories))
    if (s.notificationSettings !== undefined)
      localStorage.setItem(LS.notificationSettings, JSON.stringify(s.notificationSettings))
  } catch {
    // Ignore quota errors
  }
}
...
export async function saveSettings(s: Partial<CRMSettings>): Promise<void> {
  ...
  try {
    ...
    if (s.quarterlyTargetsByYear !== undefined)
      row.quarterly_targets_by_year = s.quarterlyTargetsByYear
    if (s.productTargetRowsByYear !== undefined)
      row.product_target_rows_by_year = s.productTargetRowsByYear
    ...
    for (let attempt = 0; attempt < 6; attempt++) {
      ...
      if (!error) break
      if (error.message?.includes('probability_defaults')) {
        delete payload.probability_defaults
        continue
      }
      if (error.message?.includes('manager_colors')) {
        delete payload.manager_colors
        continue
      }
      if (error.message?.includes('manager_territories')) {
        delete payload.manager_territories
        continue
      }
      if (error.message?.includes('notification_settings')) {
        delete payload.notification_settings
        continue
      }
      break
    }
